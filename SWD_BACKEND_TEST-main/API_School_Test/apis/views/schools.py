    ...
    def post(request, *args, **kwargs):
        ...
        try:
            if not isinstance(student_first_name,str):
                raise ValueError('Invalid First name Value and Type')
            elif  not isinstance(student_last_name,str) :
                raise ValueError('Invalid Last name Value or Type')
            elif not isinstance(subjects_title,str) :
                raise ValueError('Invalid Subjects Value or Type')
            elif isinstance(score,int) and score>=0 and score <=100 :
                raise ValueError('Invalid Score Value or Type')
            ...
            try:
                ...
                scored= StudentSubjectsScore.objects.get(student=student_id,subjects=subject_id)
            except Personnel.DoesNotExist:
                return Response('Name does not exist',status=status.HTTP_400_BAD_REQUEST)
            except Subjects.DoesNotExist:
                return Response('Subjects does not exist',status=status.HTTP_400_BAD_REQUEST)
                ...
                #Create data
                insert_credit = 0
                if subject_id in credits_context[0]["subject_id_list_that_using_this_credit"]:
                    insert_credit=credits_context[0]['credit']
                elif subject_id in credits_context[1]["subject_id_list_that_using_this_credit"]:
                    insert_credit=credits_context[1]['credit']
                elif subject_id in credits_context[2]["subject_id_list_that_using_this_credit"]:
                    insert_credit=credits_context[2]['credit']
                student = Personnel.objects.get(pk=student_id)
                subject = Subjects.objects.get(pk=subject_id)

                new_record = StudentSubjectsScore.objects.create(
                    student=student,
                    subjects=subject,
                    credit=insert_credit,
                    score=score
                )
                return Response({"id":new_record.student.id,
                                 "name":new_record.student.first_name,
            ...
            scored.score = score
            api_result.append({
                "first_name":scored.student.first_name,
                "last_name":scored.student.last_name,
                "subject_title":scored.subjects.title,
                "new_score":scored.score,
                "status":"update_success"
            })
            scored.save()
            return Response(api_result,status=status.HTTP_202_ACCEPTED)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        student_ident = Personnel.objects.get(pk=student_id)
        if student_ident.personnel_type==0:
            return Response('Role Doesn\'t for grading')
        result={
            "student":{
                "id":student_ident.id,
                "full_name":student_ident.first_name+' '+student_ident.last_name,
                "school":student_ident.school_class.school.title
            },
            "subject_detail":[
            ],"grade_point_average":None
        }
        std_subjscr= StudentSubjectsScore.objects.filter(student=student_id)
        avg_grading=0
        sum_credit=0
        
        for subject in std_subjscr:
            ...
            sum_credit+=subject.credit
            avg_grading+=grade[1]*subject.credit
        avg_grading/=sum_credit
        result['grade_point_average']=round(avg_grading,2)
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        unit_list= Personnel.objects.all().order_by('personnel_type')
        your_result = []
        i=1
        for unit in unit_list:
            if unit.school_class.school.id==school_id:
                role=''
                if unit.personnel_type==0:
                    role='Teacher, '
                elif unit.personnel_type==1:
                    role='Head of the room'
                else:
                    role='Student'
                your_result.append(
                    str(i)+'. school:'+unit.school_class.school.title+', role: '+role+'class:'+str(unit.school_class.class_order)+',name: '+unit.first_name+' '+unit.last_name
                )
                i+=1
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for school in school_list:
            school_hierarchy[school.id]={
                'school':school.title
            }
            school_classes = Classes.objects.filter(school=school.id)
            for classroom in school_classes:
                class_te = Personnel.objects.filter(school_class=classroom.id,personnel_type=0)
                class_header= Personnel.objects.filter(school_class=classroom.id,personnel_type=1)
                class_unit= Personnel.objects.filter(school_class=classroom.id,personnel_type=2)
                school_hierarchy[school.id]['class '+str(classroom.class_order)]={}
                for stuff in class_te:
                    school_hierarchy[school.id]['class '+str(classroom.class_order)]["Teacher: "+str(stuff.first_name+' '+stuff.last_name)]=[]
                    for c_header in class_header:
                        school_hierarchy[school.id]['class '+str(classroom.class_order)]["Teacher: "+str(stuff.first_name+' '+stuff.last_name)].append({
                            "Head of the room": c_header.first_name+' '+c_header.last_name
                        })
                    for unit in class_unit:
                        school_hierarchy[school.id]['class '+str(classroom.class_order)]["Teacher: "+str(stuff.first_name+' '+stuff.last_name)].append({
                            "Student":unit.first_name+' '+unit.last_name
                        })
        your_result = school_hierarchy.values()
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        highschool_tier = SchoolStructure.objects.filter(parent=None)
        if not highschool_tier:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        schoolstructure_dict = {}
        for tier in highschool_tier:
            schoolstructure_dict[tier.id]={
                'title':tier.title,
                'sub':[]
            }
            year_list = SchoolStructure.objects.filter(parent=tier)
            for year in year_list:
                schoolstructure_dict[tier.id]['sub'].append({
                    'title': year.title,
                    'sub':[]
                })
                year_classes = SchoolStructure.objects.filter(parent=year)
                for classroom in year_classes:
                    schoolstructure_dict[tier.id]['sub'][-1]['sub'].append({
                        'title':classroom.title
                    })
        your_result = list(schoolstructure_dict.values())
        return Response(your_result, status=status.HTTP_200_OK)
